...
export default function ServiceFlipCard({ service }: { service: Service }) {
    ...
    return (
        <div className="w-full">
            {/* Mobile View: Direct Link (No Flip) */}
            <Link
                href={`/services/${service.slug}`}
                className="md:hidden group relative flex flex-col justify-between h-[320px] rounded-2xl bg-zinc-900/40 border border-white/10 p-8 backdrop-blur-md overflow-hidden transition-all hover:bg-zinc-900/60 hover:border-indigo-500/50"
            >
                ...
                <div className="relative z-10 flex flex-col h-full justify-between">
                    <div>
                        ...
                        <p className="text-sm font-medium text-indigo-400 uppercase tracking-wider mb-4">{service.subtitle}</p>
                        <p className="text-sm text-zinc-400 line-clamp-3">{service.description}</p>
                    </div>
                    <div className="flex items-center gap-2 text-sm font-bold text-white">
                        See Details
                        <ArrowRightIcon className="h-4 w-4 text-indigo-400 group-hover:translate-x-1 transition-transform" />
                    </div>
                </div>
            </Link>
            ...
            <div
                className="hidden md:block group h-[320px] w-full [perspective:1000px] cursor-pointer"
            ...
            >
                <motion.div
                    ...
                    className="relative h-full w-full shadow-xl [transform-style:preserve-3d]"
                >
                    {/* Front Face */}
